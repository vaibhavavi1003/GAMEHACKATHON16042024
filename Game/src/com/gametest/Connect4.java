...
public class Connect4 {
    ...
    public Connect4() {
        ...
        board.setBackground(new Color(240, 240, 240));
        board.fisrtPlayer = 1;
...
class Connect4Board extends JPanel {
    ...
    private final Color CIRCLE_COLOR = new Color(183, 187, 190);
    private static final Color ORANGE_COLOR = new Color(255, 130, 64);
    private static final Color BLUE_COLOR = new Color(103, 200, 255);
    ...
    public void replay(String moves) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            for (int j = 0; j <= row; j++){
                if (player == 1)
                    board.get(j).set(column, ORANGE_COLOR);
                else
                    board.get(j).set(column, BLUE_COLOR);
                paintImmediately(0, 0, getWidth(), getHeight());
                if (j == row){
                    break;
                }
    ...
    private void addRedChecker(int col) {
        ...
        for (int i = 0; i <= row; i++){
            board.get(i).set(col, BLUE_COLOR);
            paintImmediately(0, 0, getWidth(), getHeight());
            if (i == row){
                break;
            }
    ...
    private void addYellowChecker(int col) {
        ...
        for (int i = 0; i <= row; i++){
            board.get(i).set(col, ORANGE_COLOR);
            paintImmediately(0, 0, getWidth(), getHeight());
            if (i == row){
                break;
            }
    ...
    private void makeMove(int col, int player) {
        for (int row = ROWS - 1; row >= 0; row--) {
            if (board.get(row).get(col) == CIRCLE_COLOR) {
                if  (player == 1)
                    board.get(row).set(col, ORANGE_COLOR);
                else
                    board.get(row).set(col, BLUE_COLOR);
                break;
            }
        }
    }
    ...
    private int evaluateWindow(List<Color> window) {
        ...
        for (int i = 0; i < 4; i++) {
            if (window.get(i) == ORANGE_COLOR) {
                aiCount++;
            } else if (window.get(i) == BLUE_COLOR) {
                humanCount++;
            }
        }
